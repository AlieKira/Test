...
namespace GameServer.Controller
{
    class UserController:BaseController
    {
        ...
        {
            string[] strs = data.Split(',');
            int username = int.Parse(strs[0]);
            int password =int.Parse(strs[1]);
            

            User user = new User(username, username.ToString(), password.ToString());
            Result reuslt=new Result(username,username,0,0,"n");
            //Result result = resultDao.GetResultByUserID(client.conn, user.userID);
            //if (result.ID == -1)
            //{
            //    resultDao.UpdateOrAddResult(client.conn, result);
            //}
            client.SetUserData(user, reuslt);
            return string.Format("{0},{1},{2},{3},{4},{5}", (int)ReturnCode.Success, user.userID, user.username, reuslt.totalCount,
                reuslt.winCount, reuslt.headPortraitPath);
        }
        ...
        public string SetHeadPortrait(string data, Client client, Server server)
        {
            ...
            client.result.headPortraitPath = strs[1];
            return ((int)ReturnCode.Success).ToString() + "," + client.result.headPortraitPath;
        }
    }
}
